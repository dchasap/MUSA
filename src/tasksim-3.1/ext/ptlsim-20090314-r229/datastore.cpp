...
DataStoreNode* DataStoreNodeLinkManager::objof(selflistlink* link) {
  return ptlsim_baseof(DataStoreNode, hashlink, link);
}
    ...
    if (count == 1) {
      ...
      if (histogramarray) {
        ...
        foreach (i, count) {
          ...
          minvalue = std::min((W64)values[i].w, minvalue);
          maxvalue = std::max((W64)values[i].w, maxvalue);
        }

        double average = double(weightedsum) / double(total);
        W64 thresh = std::max((W64)math::ceil((double)total * printinfo.histogram_thresh), (W64)1);
        W64 base = histomin;
        int width = digits(std::max(histomin, histomax));
        int valuewidth = digits(maxvalue);
        int w = std::max(width, valuewidth);
