...
//arico - changing this to be used by tracing tool
static const int MAX_BB_BYTES = 4*2048; //127 - used for insnbuf array in decoder::translate
static const int MAX_BB_X86_INSNS = 4*1024; //60 - set it to high num. - only used on check
static const int MAX_BB_UOPS = 4*512; //63; - set to lower num. - used for BB transop array size
...
struct BasicBlock: public BasicBlockBase {
  ...
  BasicBlock() : transops(new TransOp[MAX_BB_UOPS*2]) {}
  ~BasicBlock() {delete[] transops;}
  void reset();
