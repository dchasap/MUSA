...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
  ...
  $Date:: 2015-02-17 17:0#$:  Date of last commit
...
void TASK_Init(int sintetic_io_applications)
{
       ...
       Ptask  = (struct t_Ptask *) next_queue (&Ptask_queue))
  {
    ...
    for (tasks_it = 0; tasks_it < Ptask->tasks_count; tasks_it++)
    {
      ...
      // CAA: Tasksim first phase initialization.
      if (is_tasksim_enabled()){
        tasksim_init_task(task);
      }
...
void TASK_New_Task(struct t_Ptask *Ptask, int taskid, int nodeid)
{
  ...
  // CAA DEBUG LULESH 
  // open a file descriptor to write de debug logs
  if (enable_fdebug) {
    int f_sz = 255;
    char file_name[f_sz];
    sprintf(file_name,"log_integration_rank_%d", task->taskid);
    task->fddebug = fopen(file_name, "w+");
  }
                 ...
                 unsigned long int module_type)
{
      ...
      (thread->global_op_done == FALSE))
  {
        ...
        1 + count_queue (&(thread->task->Ptask->global_operation)))
    {
      ...
      //printf("CAA_ ti to remote_Startup in module exit\n");
