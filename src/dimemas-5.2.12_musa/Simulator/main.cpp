...
#define TASKSIM_ENABLED
#ifdef TASKSIM_ENABLED
...
#ifdef TASKSIM_ENABLED
  // CAA:
  printf ("\t-tasksim\tUNDER DEVELOPEMENT computation replacement performed by TaskSim burst/detailed simulations'\n");
#endif
...
void parse_arguments(int argc, char *argv[])
{
    ...
    for (j = 1; (j < argc - 1) && (argv[j][0] == '-'); j++)
    {
      switch (argv[j][1])
      {
        ...
        case 'S':
          ...
          break;
        case 't':
          // CAA: añadimos la funcionalidad de Tasksim
          if (!strcmp(argv[j], "-tasksim")) {
                   j++;
              // argument is mode: must be online or presim
              tasksim_enable(argv[j]);
          } else if (!strcmp(argv[j], "-ts_trace_folder")) {
                   j++;
                   tasksim_enable_trace_folder(argv[j]);
          } else if (!strcmp(argv[j], "-ts_trace_name")) {
                   j++;
                   tasksim_enable_trace_name(argv[j]);
          } else if  (!strcmp(argv[j], "-ts_config")) {
                   j++;
                   tasksim_enable_config(argv[j]);
          } else if (!strcmp(argv[j],  "-ts_presim_file")) {
                   j++;
                   tasksim_enable_presim_file(argv[j]);
          } else if (!strcmp(argv[j], "-ts_prv")) {
                   j++;
                   tasksim_enable_paraver(argv[j]);
          } else { 
  ...
  // tasksim needs to evalute if the combination of parameters is correct.
  // online and presim, with full arguments.
  // presim --> trace presim_file {prv}
  // online --> trace config_file {prv}
  tasksim_check_options();
...
// should not break the program??
  ...
  // caa: location to check arguments
  //if (is_tasksim_enabled()) TASKSIM_Init();
