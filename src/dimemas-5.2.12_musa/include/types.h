...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
  ...
  $URL:: https://svn.bsc.es/repos/DIMEMAS/trunk/i#$:  File
  $Rev:: 193                                      $:  Revision of last commit
  $Author:: jgonzale                              $:  Author of last commit
  $Date:: 2015-02-17 17:01:02 +0100 (mar 17 de fe#$:  Date of last commit
...
/// CAA TaskSim plugin
typedef struct tsim_config tsim_config;
typedef struct tsim_phase tsim_phase;
typedef struct tsim_phase_sim tsim_phase_sim;
///////////////////////////////
struct tsim_phase_sim {
  int             to_simulate;
  int             pending;
  int             mode;
  int             presimulated;
  char            in_config_file[250];
  char            paraver_file[250];
  char            out_file[250];
  int             in_config_exists;
  int             paraver_exists;
  int             out_file_exists;
  double          time;
};
...
///////////////////////////////
struct tsim_config {
  char*             filename_effective_config;
  char*             filename_config;
  char*             filename_relation;
  char*             filename_default_trace;
  char*             filename_streaminfo;
  char*             full_trace_prefix;
  char*             prefix_name;
  long              range_ini;
  long              range_end;
  long              mem_ini;
...
/*Vladimir: for optimization, when a task has many threads*/
  ...
  // CAA: tasksim related
  tsim_config       sim_config;
  tsim_phase*       phase_init;
  tsim_phase*       phase_end;
  tsim_phase*       phase_current;
  double            phase_counter;


  tsim_phase*       phase_to_replace;
  int               in_ompss;
  int               pending_replacement;


  // CAA: debug.. output_file
  FILE *fddebug;
