...
for corelist in 1 2 4 8 16; do
    sufix="_"$corelist"cores"
    ...
    for vec in $vector_list; do
        ...
        for config in $config_list; do
            for cpus in $corelist; do
                ...
                sim_ref_log_file=./logs/simulation_iso_sve_ompss_${configA}_${cpus}cores_${scenario}_vec_128.txt
    ...
    bw_plottitle="Simulation bandwidth for ${corelist} cores [GB/s]"
    sim_plottitle="Total executed time for ${corelist} cores [normalized]\n{/*0.8 (normalized to configuration A ${corelist} cores SVE 128 bits)}"
