...
#define MAX_PROC_MAPS_SIZE 16*1024*1024
  ...
  while (p && (*p)) {
    ...
    bool stack = (((pattr && strequal(pattr, "[stack]")) || 
                   (line == (linecount-2))) ? (MAP_STACK!=0) : false);
...
void save_context_switch_to_sim_lowlevel() {}
...
extern "C" void external_signal_callback(int sig, siginfo_t* si, void* contextp) {
  ...
  default:
    if (logfile) {
      logfile << "Warning: unknown signal ", si->si_signo, "; ignoring", endl, flush;
      break;
    }
  }
}
...
extern "C" void* ptlsim_preinit(void* origrsp, void* nextinit) {
  ...
  //
  user_stack_size = std::min((W64)rlimit.rlim_cur, (W64)128*1024*1024);
