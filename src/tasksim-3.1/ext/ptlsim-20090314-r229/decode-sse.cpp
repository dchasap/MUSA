...
bool TraceDecoder::decode_sse() {
  ...
  // Francesc: duplicated this decoding options to also handle the 0x03 prefix used by PTLSim
  //
...
#undef Q
  ...
  case 0x371: // psxxw imm8
  case 0x372: // psxxd imm8
  case 0x373: { // psxxq imm8 or psrldq|pslldq imm8 (byte count)
    ...
    if unlikely ((op == 0x373) && (lowbits(modrm.reg, 2) == 3)) {
      EndOfDecode();
      //
      // pslldq: (left)
      //
...
#undef Q
  ...
  case 0x2b8: {
    ...
    DECODE(gform, ra, d_mode);
    EndOfDecode();
    int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
    int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
    this << TransOp(OP_maskb, rdreg, REG_zero, rareg, REG_zero, 3);
    break; 
  }
  case 0x53a: {
    byte op2 = fetch1();
    switch (op2) {
      ...
      case 0x63: {
        ...
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        if (op2 == 0x63) {
          this << TransOp(OP_vcmp, REG_rcx, rdreg+0, rareg+0, REG_imm, 3);
          this << TransOp(OP_vcmp, REG_rcx, rdreg+1, rareg+1, REG_imm, 3);
        } else {
          this << TransOp(OP_vcmp, REG_xmml0, rdreg+0, rareg+0, REG_imm, 3);
          this << TransOp(OP_vcmp, REG_xmmh0, rdreg+1, rareg+1, REG_imm, 3);
        }
        break;
      }
      case 0x0C: {
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_vshr, rdreg+0, rdreg+0, rareg+0, REG_imm, 3);
        this << TransOp(OP_vshr, rdreg+1, rdreg+1, rareg+1, REG_imm, 3);
        break;
      }
      case 0x0F: {
        // PALIGNR
        ...
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_vshr, REG_xmml0, rdreg+0, rareg+0, REG_imm, 3);
        this << TransOp(OP_vshr, REG_xmmh0, rdreg+1, rareg+1, REG_imm, 3);
        break;
      }
      case 0x16: {
        // PEXTRD
      ...
      default: {
        // printf("Cannot decode instruction 0F 3A %02X.\n", op2);
        MakeInvalid();
        break;
      }
    }
  ...
  case 0x538: {
    ...
    switch(op2){
      case 0x00: {
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_permb, rdreg+0, rdreg+0, rareg+0, rareg+1, 3);
        if (op == 0x538) {
          this << TransOp(OP_permb, rdreg+1, rdreg+1, rareg+0, rareg+1, 3);
        }
        break;
      }
      case 0x02: {
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_vadd, rdreg+0, rdreg+0, rareg+0, rareg+1, 3);
        this << TransOp(OP_vadd, rdreg+1, rdreg+1, rareg+0, rareg+1, 3);
        break;
      }
      case 0x04: {
        // PMADDUSW
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_vmaddp, rdreg+0, rdreg+0, rareg+0, rareg+1, 3);
        this << TransOp(OP_vmaddp, rdreg+1, rdreg+1, rareg+0, rareg+1, 3);
        break;
      }
      case 0x10: {
        // PBLENDVB
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_permb, rdreg+0, rdreg+0, rareg+0, REG_xmml0, 3);
        this << TransOp(OP_permb, rdreg+1, rdreg+1, rareg+1, REG_xmmh0, 3);
        break;
      }
      case 0x17: {
        // PTEST
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_vcmp, rdreg+0, rdreg+0, rareg+0, REG_xmml0, 3);
        this << TransOp(OP_vcmp, rdreg+1, rdreg+1, rareg+1, REG_xmmh0, 3);
        break;
      }
      case 0x40: {
        // PMULLD
        ...
        int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
        int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
        this << TransOp(OP_vmull, rdreg+0, rdreg+0, rareg+0, rareg+1, 3);
        this << TransOp(OP_vmull, rdreg+1, rdreg+1, rareg+0, rareg+1, 3);
        break;
      }
      default:
        // printf("Cannot decode instruction 0F 38 %02X.\n", op2);
        MakeInvalid();
        break;
    }
    break;
  ...
  default: {
    // printf("Cannot decode instruction 0F %02X.\n", op);
    MakeInvalid();
    break;
  }
  }
