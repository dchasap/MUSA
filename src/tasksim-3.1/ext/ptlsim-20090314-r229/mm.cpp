...
struct ExtentAllocator {
  ...
  int extent_size_to_slot(size_t size) const {
    return std::min(size-1, (size_t)(SIZESLOTS-1));
  }
  ...
  void* alloc(size_t size) {
    ...
    peak_bytes_allocated = std::max(peak_bytes_allocated, current_bytes_allocated);
  ...
  void free(void* p, size_t size) {
    ...
    frees++;
    current_bytes_allocated = std::min(current_bytes_allocated - (size * CHUNKSIZE), 0ULL);
...
struct SlabAllocator {
  ...
  FreeObjectHeader* alloc_from_page(PageHeader* page) {  
    ...
    peak_objs_allocated = std::max(current_objs_allocated, peak_objs_allocated);

    current_bytes_allocated += objsize;
    peak_bytes_allocated = std::max(current_bytes_allocated, peak_bytes_allocated);
  ...
  PageHeader* alloc_new_page() {
    ...
    peak_pages_allocated = std::max(current_pages_allocated, peak_pages_allocated);
  ...
  void free(void* p) {
    ...
    current_bytes_allocated -= std::min((W64)objsize, current_bytes_allocated);
...
void* ptl_mm_try_alloc_private_pages(Waddr bytecount, int prot, Waddr base, void* caller) {
  ...
  if (addr) {
    ...
    pagealloc.peak_bytes_allocated = std::max(pagealloc.peak_bytes_allocated, pagealloc.current_bytes_allocated);
  }
...
void ptl_mm_free_private_pages(void* addr, Waddr bytecount) {
  ...
  pagealloc.frees++;
  pagealloc.current_bytes_allocated -= std::min(pagealloc.current_bytes_allocated, (W64)bytecount);
...
void* ptl_mm_alloc(size_t bytes, void* caller) {
  ...
  if likely ((bytes <= SLAB_ALLOC_LARGE_OBJ_THRESH)) {
    ...
    if unlikely (!p) {
      ...
      Waddr pagebytes = std::max((Waddr)ceil(bytes, PAGE_SIZE), (Waddr)GEN_ALLOC_CHUNK_SIZE);
      //
      // We need the pages in the low 2 GB of the address space so we can use
